...
function createViewTopLevelStmts(view, targetStatements) {
    ...
    const /** @type {?} */ renderCompTypeVar = o.variable(`renderType_${identifierName(view.component.type)}`); // fix highlighting: `
    if (view.viewIndex === 0) {
        ...
        targetStatements.push(renderCompTypeVar
            ...
            view.styles,
            o.literalMap(view.animations.map((entry) => [entry.name, entry.fnExp])),
        ]))
...
function generateDetectChangesMethod(view) {
    ...
    const /** @type {?} */ varStmts = createSharedBindingVariablesIfNeeded(stmts);
